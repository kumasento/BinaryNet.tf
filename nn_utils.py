...
def binarize(x):
  """ Clip and binarize tensor using the straight through
    estimator (STE) for the gradient.

  Straigh-Through Esitamtor here means we don't compute the
  gradient of sign.
  It is not the original form of STE, but the one that uses
  hard tanh.

  The input tensor is first clipped within [-1, 1]
  and then get its sign.
  The sign operator's gradient is overwriten by Identity.
  ...
  with ops.name_scope("Binarized", [x]) as name:
    with g.gradient_override_map({"Sign": "Identity"}):
      x = tf.clip_by_value(x, -1, 1)
      return tf.sign(x)


def stochastic_binarize(x, name=None):
  """ Binarize the tensor by a stochastic mechanism.

  May improve the implementation by using uniform distribution.

  Args:
    x: input tensor
    name: name of the generated operator.

  Returns:
    A tensor of the binarized value, and a tensor of the probabilities.
  """
  ...
  def affineLayer(x, is_training=True):
    with tf.variable_op_scope([x], name, 'Affine', reuse=reuse):
      ...
      w = tf.get_variable('weight', [
                          nInputPlane, nOutputPlane], initializer=tf.contrib.layers.xavier_initializer())
